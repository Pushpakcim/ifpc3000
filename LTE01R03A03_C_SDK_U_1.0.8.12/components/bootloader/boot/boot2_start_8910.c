...
void bootStart(uint32_t param)
{
    ...
    bootResetPinEnable();
    hwp_iomux->pad_gpio_8_cfg_reg  = 0;        //set GPIO_8 as gpio function
    hwp_gpio1->gpio_oen_set_out = (1 << 8);  //set GPIO_8 output mode
    hwp_gpio1->gpio_clr_reg = (1 << 8);      //set GPIO_8 output LOW level
  //  delay_ms(5000);        // delay
